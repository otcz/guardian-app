...
export class OrganizationListComponent implements OnInit {
  ...
  load() {
    ...
    this.orgService.list().subscribe({
      next: (data) => { this.orgs = data || []; this.applyFilter(); this.loading = false; },
      error: (e) => { this.error = e?.error?.message || 'Error al cargar organizaciones'; this.loading = false; }
    });
  }

  applyFilter() {
    const f = (this.filter || '').trim().toLowerCase();
    if (!f) { this.filtered = [...this.orgs]; return; }
    this.filtered = this.orgs.filter(o => (o.nombre || '').toLowerCase().includes(f));
  }

  goCreate() { this.router.navigate(['/crear-organizacion']); }
