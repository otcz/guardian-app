...
export class OrganizationAuditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.orgId) { this.error = 'No se ha definido organización'; return; }
    this.load();
  }

  ngOnDestroy() { if (this.intervalRef) clearInterval(this.intervalRef); }

  toggleAuto() {
    if (this.autoRefresh) {
      this.intervalRef = setInterval(() => this.load(true), 10000);
    } else if (this.intervalRef) { clearInterval(this.intervalRef); }
  }

  applyFilter() {
    const f = (this.filter || '').toLowerCase().trim();
    if (!f) { this.filtered = [...this.logs]; return; }
  ...
  load(silent = false) {
    ...
    this.http.get<AuditLog[]>(url).subscribe({
      next: data => { this.logs = data || []; this.applyFilter(); this.loading = false; },
      error: e => { this.error = e?.error?.message || 'No se pudieron cargar los registros'; this.loading = false; }
    });
  }
